...
import { z } from "zod";
import { SKIP_PATTERNS } from "../lib/const";
import {
  fetchAllPRFiles,
  fetchFileContent,
  githubFetch,
...
import { fileSchema, prIdentifierSchema, prSchema } from "../lib/schema";
...
export const getPullRequestFiles = createTool({
  ...
  execute: async (inputData) => {
    const { owner, repo, pullNumber, page = 1 } = inputData;
...
export const getFileContent = createTool({
  ...
  inputSchema: z.object({
    ...
    ref: z
      .string()
      .describe("Git ref (branch, tag, or commit SHA). Prefer commit SHA."),
  }),
  outputSchema: z.object({
  ...
  execute: async (inputData) => {
    ...
    if (!result) {
      return { content: null, encoding: "utf-8", size: 0 };
    }
    return result;
  },
});
